...
class EncoderDecoderBase():
    def __init__(self, state, rng, parent):
        ...
        self.dialogue_rec_activation = eval(self.dialogue_rec_activation) # what does it mean ?
        self.sent_rec_activation = eval(self.sent_rec_activation) # what does it mean ?
...
class UtteranceEncoder(EncoderDecoderBase):
    ...
    GRU: r(t) = sigmoid(Wr * x(t) + Ur * h(t-1) + br) forget gate
         z(t) = sigmoid(Wz * x(t) + Uz * h(t-1) + bz) update gate
         h'(t) = phi(W * x(t) + U * (r * h(t-1)) + bhh)
         h(t) = z(t) * h'(t) + (1 - z(t)) * h(t)
    len(h) = qdim_encoder
    len(x) = rankdim
    """
    ...
    def plain_sent_step(self, x_t, m_t, *args):
        args = iter(args)
        h_tm1 = next(args)
        ## m_t is a shared variable, 0 means keep the same dimension, x means increase a new dimension with length 1
        if m_t.ndim >= 1:
        ...
        if self.reset_utterance_encoder_at_end_of_utterance:
            ## this will let h become zero everytime we encounter </s>
            hr_tm1 = m_t * h_tm1
        else:
            hr_tm1 = h_tm1
    ...
    def GRU_sent_step(self, x_t, m_t, *args): ## the calculation of RNN in sentence level
        args = iter(args)
        h_tm1 = next(args)
    ...
    def build_encoder(self, x, xmask=None, prev_state=None, **kwargs):
        ...
        ## after this, x.dim becomes 3
        xe = self.approx_embedder(x)
        if xmask == None:
            ## calculate the result of (x != self.eos_sym), find the location of </s>
...
class DialogEncoder(EncoderDecoderBase):
    ...
    def init_params(self):
        ...
        if self.deep_dialogue_input:
            ## what is the usage of deep_dialogue_input(turn utterance vector into sdim size at first)
...
class DialogDummyEncoder(EncoderDecoderBase):
    ...
    ## transfer word level hidden state h into utterance level hidden state h, used for latent variable
    """
...
class DialogEncoderDecoder(Model):
    ...
    def compute_updates(self, training_cost, params):
        ...
        ## if grads = nan/inf, grads = original parameter
    ...
    def build_next_probs_function(self):
        if not hasattr(self, 'next_probs_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                ## latent_utterance_variable_prior_encoder is an instance of DialogLevelLatentEncoder
    ...
    def build_encoder_function(self):
        if not hasattr(self, 'encoder_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                ## latent_utterance_variable_approx_posterior_encoder is an instance of
    ...
    def __init__(self, state):
        ...
        if state['collaps_to_standard_rnn']:
            ...
            ## do not understand the usage of this
