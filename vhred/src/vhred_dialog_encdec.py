...
class TwoLayerMLP(EncoderDecoderBase):
    ...
    def build_output(self, inp, bnmask):
        ...
        if self.normop_type == 'BN':
            ...
            ## means the history estimation of mean, var
            ...
            ## means the newly updated mean and var
...
class UtteranceEncoder(EncoderDecoderBase):
    ...
    def build_encoder(self, x, xmask=None, bnmask=None, prev_state=None, **kwargs):
        ...
        ## batch mask, need to pay attention
        bnmask_given = True
        if bnmask == None:
            bnmask_given = False
...
class DialogEncoder(EncoderDecoderBase):
    ...
    def GRU_dialogue_step(self, h_t, m_t, bnmask_t, hs_tm1, *args):
        ...
        ## add an overwrite operation. os_t = 1 means using overwrite instead of h_tilde
        ...
        if self.normop_type == 'BN':
            if self.dialogue_encoder_gating == "GRU":
                if (not one_step) and (h.ndim == 3) and (bnmask_given):
                    ...
                    ## means the num of batch in each timestep
...
class UtteranceDecoder(EncoderDecoderBase):
    ...
    def init_params(self):
        ...
        ## suppose idim is the dimension of vocabulary
        ...
        if self.utterance_decoder_gating == "GRU":
            ...
            ## LSTM:
            ## f(t) = Wfh * h(t-1) + Wfx * x(t) + bf
            ## i(t) = Wih * h(t-1) + Wix * x(t) + bi
            ## o(t) = Woh * h(t-1) + Wox * x(t) + bo
            ## g(t) = Wgh * h(t-1) + Wgb * x(t) + bg
            ## c(t) = sigmoid(f(t)) * c(t-1) + sigmoid(i(t)) * tanh(g(t))
            ## h(t) = sigmoid(g(t)) * tanh(c(t))
            # Input gate
    ...
    def build_output_layer(self, hs, xd, hd):
        ...
        ## here pre_activ.shape = (bs, out_target_dim)/(length, bs, out_target_dim), reshape into (bs, out/2, 2)/(length, bs, out/2, 2)
        ## what is the usage of max_out?
        if self.maxout_out:
    ...
    def output_nce(self, pre_activ, y, y_hat):
        ## need t opay attention about the shape of y and y_hat
    ...
    def build_decoder(self, decoder_inp, x, xmask=None, xdropmask=None, y=None, y_neg=None, mode=EVALUATION, prev_state=None, step_num=None):
        ...
        ## why do we need to reorder the dimension?
        if xd.ndim != 3:
        ...
        if self.utterance_decoder_gating == "" or self.utterance_decoder_gating == "NONE" \
            ...
            if self.utterance_decoder_gating == "LSTM":
                ...
                o_dec_info = [hd_init]
                ## need to figure out what does selective option mean?
                if self.decoder_bias_type == "selective":
            ...
            if isinstance(_res, list) or isinstance(_res, tuple):
                ...
                hd = _res
            ## hd is the result of decoder(timestep, bs, q_dim)
            # OBSOLETE:
        ...
        ## pre_activ.shape = (timestep, bs, idim)
        ...
        if mode == UtteranceDecoder.EVALUATION:
            ...
            ## output.shape = (timestep, bs, idim)
            target_probs = GrabProbs(outputs, y)
            ## target_probs = (1,timestep*bs)? representing prob for each word
    ...
    def LSTM_step(self, xd_t, m_t, decoder_inp_t, hd_tm1):
        ## LSTM decoder:
        ## f(t) = Wfh * h(t-1) + Wfx * x(t) + Wfs * dec_inp + Wfc * c(t-1) + bf
        ## i(t) = Wih * h(t-1) + Wix * x(t) + Wis * dec_inp + Wic * c(t-1) + bi
        ## o(t) = Woh * h(t-1) + Wox * x(t) + Wos * dec_inp + Woc * c(t-1) + bo
        ## c(t) = sigmoid(f(t)) * c(t-1) + sigmoid(i(t)) * tanh(Wx * x(t) + Ws * dec_inp + Wc * c(t-1) + b)
        ...
        # RNN receives the decoder_inp_t vector as bias without modification
            ...
            ## calculate without considering dec_inp
    ...
    def GRU_step(self, xd_t, m_t, decoder_inp_t, hd_tm1):
        ...
        ## h'(t) = tanh(Wx * x(t) + Wh * (r * h(t-1)) + Ws * dec_inp + b)
...
class DialogEncoderDecoder(Model):
    ...
    def __init__(self, state):
        ...
        ## guess the dictionary seems like: ['a', 0, noise_prob(freq), document_freq]
        ...
        if self.direct_connection_between_encoders_and_decoder:
            ## using hs_dummy(computed by h) as part of the input
        ...
        ## target_probs is the prob for y, full_matrix is the output of softmax
        ...
        ## pay attention to the value of reset mask
